...
table3proper <- table3 |>
  ...
  mutate(
    ...
    # "unknown" education shares that round to 0 are shown to one decimal so
    # they don't visually disappear (e.g. 0.4% in 2021); low/mid/high keep
    # their integer rounding and sum to 100. See CHANGELOG 2026-06-19.
    prop = if_else(
      prop_type == "unknown" & prop_int == 0,
      round(prop * 100, 1),
      prop_int
    )
  ) |>
...
table5 <- analysis_sample |>
  ...
  ungroup() |>
  mutate(
    variable = "Intention to marry within 3 years",
    value_label = case_match(int3, 1 ~ "Yes", 0 ~ "No")
  ) |>
  ...
  bind_rows(
    analysis_sample |>
      ...
      mutate(
        variable = "Have trouble making ends meet",
        value_label = case_match(econ, 1 ~ "Yes", 0 ~ "No")
      ) |>
  ...
  bind_rows(
    analysis_sample |>
      ...
      ungroup() |>
      mutate(
        variable = "Agrees marriage is outdated",
        value_label = case_match(att, 1 ~ "Yes", 2 ~ "No", 3 ~ "Neutral")
      ) |>
...
print(doc, target = file.path("tables", "Table_A1.docx"))
```
...
agestat_rows   <- df_formatted |> filter(variable == "Age stat")  |> select(-variable, -starts_with("sd_"))
...
df_final <- bind_rows(
  ...
  make_header_row("Age"),       agestat_rows, age_rows,
...
doc <- read_docx() |> body_add_flextable(ft)
print(doc, target = file.path("tables", "Table_A2.docx"))
```
...
doc <- read_docx() |> body_add_flextable(ft)
print(doc, target = file.path("tables", "Table_2.docx"))
```

```{r table-5-export}
table_data <- table5 |>
  mutate(value_label = factor(value_label, levels = c("Yes", "No", "Neutral"))) |>
  pivot_wider(names_from = survey, values_from = prop) |>
  select(variable, value_label, `2005`, `2021`) |>
  arrange(variable, value_label) |>
  # cast to double so colformat_double's "%" suffix applies (round_to_100
  # returns integers, which colformat_double silently skips)
  mutate(across(c(`2005`, `2021`), as.double))
...
doc <- read_docx() |> body_add_flextable(ft)
print(doc, target = file.path("tables", "Table_A3.docx"))
```
