...
round1tidy <- round1 |> 
  ...
  mutate(
    parstat = case_when(
      aparstat == 1 & (amarstat == 1 | amarstat == 3 | amarstat == 4) ~ 1, # cohabting
...
analysis <- analysis |> 
  mutate(
    att = case_when(
      att03a == 4 | att03a == 5 ~ 1, # agree
      att03a == 1 | att03a == 2 ~ 2, # disagree
      att03a == 3 ~ 3, # indifferent
      TRUE ~ NA
    ),
    int3 = case_when(
      dem28c == 4 | dem28c == 5 ~ 1, # yes
      dem28c == 1 | dem28c == 2 | dem28c == 3 ~ 0, # no
      TRUE ~ NA
    ),
    econ = case_when(
...
analysis_sample <- analysis |> 
  filter(age4 < 4 & parstat == 1 & cores == 1 & !(is.na(edu3)) & !(is.na(typ))) 
