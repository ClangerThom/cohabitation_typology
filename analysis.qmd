...
analysis_sample <- analysis |> 
  filter(age < 60 & parstat == 1 & !(is.na(edu3)) & edu3 != "unknown" & !(is.na(typ))) 
# not missing on typology, education or partnership status. Cohabitant and under the age of 60, 
...
figure5 <- analysis_sample |> 
  summarise(
    weight = sum(weight),
    .by = c("survey", "age3" , "typ")
  ) |> 
  mutate(
    total_weight = sum(weight), .by = c("survey", "age3")
  ) |> 
  mutate(
    prop = weight/total_weight
  ) |> 
