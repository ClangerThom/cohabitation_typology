...
figure_4 <- 
  ...
  scale_x_continuous(breaks = seq(0, 35, 10), expand = expansion(mult = c(0.1, 0.2))) +
...
figure_4a <- 
  ...
  # legend
  geom_text(aes(label = year, color = year), 
            data=. |>  filter(type == "Forerunners - most likely getting married"),
            nudge_y =.25, 
            fontface="bold",
            size=3.25)+ 
...
ggsave(
  filename = file.path("plots", "figure_4_aggr.png"),
  plot = p_whole_4a,
  width = 4.9,
  height = 3, 
  units = "in",
